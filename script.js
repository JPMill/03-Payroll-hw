...
const collectEmployees = function () {
  ...
  const salaryString = prompt("Enter Salary:")
  const employeeSalary = parseInt(salaryString)
  const employeeObjects = {firstName: `${firstName}`, lastName: `${lastName}`, salary: `${employeeSalary}`}
...
const getRandomEmployee = function (employeesArray) {
  ...
  const randomEmployee = collectEmployees[Math.floor(Math.random() * collectEmployees.length)];
